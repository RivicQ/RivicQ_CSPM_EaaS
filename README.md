...
| **Quantum Security Fabric (labeled demo)** | https://rivicq.github.io/RivicQ_CSPM_EaaS/nexus/ |
...
The Community console is the **RivicQ Security Cloud** — dark-native zinc chrome (`#09090B` / `#18181B`) with Inter and JetBrains Mono. Light mode remains available. **Quantum Security Fabric** (wordmark **FABRIC**, previously labeled NEXUS) is a separate, original security-graph demo at `/nexus` (obsidian fabric; labeled synthetic data; no secret values). Control mappings are not certifications.
...
Separate original identity (obsidian fabric, wordmark FABRIC). Discovery, each detection (`QSF-*`), attack stages, and domain inventories have their own graphs. Synthetic fixture only — no secret values. See [docs/NEXUS.md](docs/NEXUS.md).
...
web/                    React SaaS (sky-blue and white theme)
