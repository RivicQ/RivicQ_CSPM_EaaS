...
```
quantum-sdk/internal/quantum/ibmquantum/provider.go   # reference implementation
```

Steps:

1. Implement `provider.QuantumProvider` in a new package
   (`quantum-sdk/internal/quantum/<provider>/`).
...
4. Register the factory in `quantum-sdk/internal/quantum/builtin/builtin.go` behind an
...
- `quantum-sdk/internal/quantum/ibmquantum/provider.go` — IBM Quantum (wraps
  `quantum.IBMQuantumClient`).
- `quantum-sdk/internal/quantum/oqsengine/engine.go` — Open Quantum Safe (software + liboqs
...
- `quantum-sdk/internal/quantum/nistpqc/provider.go` — dependency-free FIPS 203/204/205
  validation engine.
