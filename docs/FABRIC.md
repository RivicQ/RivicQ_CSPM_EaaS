...
Labeled security-graph demo. **Not** a reskin of the Community console, and **not** a certification of this repository or of any customer.

Formerly called **NEXUS** and **Quantum Security Fabric** (wordmark FABRIC). Source stays in [`fabric/`](../fabric/). Pages path stays `/fabric`. `/nexus` redirects here.
...
| Detections (`QSF-*`) | One subgraph per finding (IDs are historical; the product name is RivicQ Graph) |
...
| Overview, posture, compliance, analyst | Risk, drill, control, and attack-path graphs |
...
- Control **mappings** (NIST, ISO, SOC 2, PCI, GDPR, DORA, NIS2, BSI, CIS, FIPS). Not audits or certifications of RivicQ Graph or of RivicQ GmbH.
...
Black canvas `#000000`, surface `#0A0A0F`, violet `#7C3AED`, Inter + JetBrains Mono. Semantic status color only. Distinct graph node shapes. Glow for AI, threats, PQC, and active graph nodes. Same nebula system as the public website and Community console.
