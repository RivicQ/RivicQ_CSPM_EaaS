...
**RivicQ Graph** (wordmark **GRAPH**) is the labeled Community security-graph demo in this repository (`fabric/`, `/fabric` on GitHub Pages). It was previously called NEXUS and Quantum Security Fabric. It shares the nebula visual system (black canvas, violet `#7C3AED`). It does **not** store or display customer secrets, private keys, or production credentials. Control mappings and PQC algorithm names are not certifications or completed-migration claims. Enterprise rights remain licensed as in §1.
