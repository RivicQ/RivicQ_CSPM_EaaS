...
exports.BankingDemoApp = void 0;
const express_1 = __importDefault(require("express"));
const crypto = __importStar(require("crypto"));
const runtime_1 = require("../src/interceptor/runtime");
const generator_1 = require("../src/cbom/generator");
class BankingDemoApp {
    constructor() {
        this.transactions = [];
        this.kycData = [];
    ...
    signTransaction(transaction) {
        ...
        const result = this.interceptor.interceptCryptoCall('RSA_sign', 'rsa-2048-sign', Buffer.from(data, 'utf8'));
        return result.result.toString('base64');
    }
    encryptKYCData(data) {
        // This encryption will be monitored but AES-256 is already quantum-safe
        const result = this.interceptor.interceptCryptoCall('AES_encrypt', 'aes-256-gcm', Buffer.from(data, 'utf8'));
        return {
            data: result.result,
    ...
    getCryptoMetrics(req, res) {
        const metrics = this.interceptor.getMetrics();
        res.json(metrics);
    }
    async getCBOM(req, res) {
        try {
...
exports.BankingDemoApp = BankingDemoApp;
