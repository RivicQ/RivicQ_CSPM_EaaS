...
class RivicOperator {
    ...
    async start() {
        console.log('🚀 Starting Rivic Q-Runtime Operator...');
        // Install CRDs
        await this.installCRDs();
        // Setup admission controller webhook
    ...
    async setupAdmissionWebhook() {
        ...
        const webhook = {
            ...
            webhooks: [{
                    name: 'pod-injector.quantum.rivic.eu',
                    clientConfig: {
                        service: {
                            name: 'rivic-webhook',
        ...
        try {
            await this.admissionApi.createMutatingAdmissionWebhook({ body: webhook });
            console.log('✅ Admission webhook configured');
        }
        catch (error) {
            if (error.response?.statusCode !== 409) {
                throw error;
            }
            console.log('📋 Admission webhook already exists');
        }
    }
    watchRivicConfigs() {
